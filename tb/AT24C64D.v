...
integer k;
initial 
  for (k = 0; k <= MEM_SIZE-1; k = k+1) memory[k] = {BYTE_SIZE {1'b1}};	
...
always @(posedge SCL) 
  if (ld_addr_reg_MSB_byte == 1) begin
    `ifdef AT24C1024 
  ...
  case (MEM_SIZE)
    ...
    MEM_SIZE_1K_BIT: 	addr_reg[6:0] <= S_Byte_Shft_Reg[6: 0];
    default:		addr_reg[7:0] <= S_Byte_Shft_Reg[7: 0];
  endcase
...
always @ (negedge SCL) 
  if (shift_out == 1) S_Byte_Shft_Reg <= S_Byte_Shft_Reg << 1;
...
always @(posedge SCL) begin			  // Simplified 7/26/2008	
  if ({Protocol_Recovery_reg [7:0], SDA_in} == 9'b1_1111_1111) begin
...
initial begin				// Power-Up Initialization
    `ifdef 	AT34C02 
      AT34C02_SWP_reg = 0;		// one-time programmable
      AT34C02_SWP_enabled = 0;	// one-time programmable
    `endif
    Power_up_initialization;
    if (VERBOSE) $display("Begin Power Up Sequence");
    if (VERBOSE) $display ("Waiting for START_condition");
    S_send_rcvb = RECEIVE;
...
// condition and awaits a subsequent start condition.
  ...
  always @ (posedge SDA_in) 		// Stop condition
    if(SCL == 1) begin  
      ...
      if (VERBOSE) $display ("STOP condition", $time);
      -> STOP_condition;
      Valid_Address_flag = 0;
...
// before the falling edge of the clock.  (05/30/2008)
   ...
   always @ (START_trigger) begin: Data_Transmission	//04/24/2006
      First_START_flag = 1;
      if (VERBOSE) $display("******************* Data Transmission Triggered by start", $time);

      forever begin: Outer_Block

	 // Check for power up enabled
	 if (VERBOSE && (!RUN_flag)) begin
	    $display ("MACHINE NOT POWERED UP");
	    disable Data_Transmission;
	 end

	 // Check for time-out condition
	 if (RUN_flag && (Time_Out_flag)) begin
	    if (VERBOSE) $display("Time_Out_flag Asserted - Skip START condition"); 
	    if (VERBOSE) $display($realtime);
	    disable Data_Transmission;		// 05/30/3008 Prevent inf loop 
	    //disable Outer_Block;			// 05/30/3008 Prevent inf loop
	 end

	 else begin: Inner_Block

	    Recover_from_Protocol_Interupt; // Clear flags and variables as needed
	    if (VERBOSE) $display("Valid Start Condition - Begin Data_Transmission", $time); 
	    if (VERBOSE) $display("Getting Device Address Byte", $time);

	    Get_a_Byte;		// Reads 8 bits of data on (rising edge of SCL)
	    if(VERBOSE) $display("Byte received:  %H ",S_Byte_Shft_Reg, $time);	
	    @ (negedge SCL)  // First drop in SCL after byte is read
	      shift_in = 0;
	    NO_ACK_flag = 0;
	    if (Device_Address_Active) Check_for_Valid_Upper_Bits_of_Device_Address;
	    if (Valid_Address_flag)
	      begin:  Got_Address_Match 
		 if (VERBOSE) $display ("Device Address_Match Found", $time);
		 ...
		 endcase
		 if (VERBOSE) $display ("S_Byte_Shft_Reg[0] = ", S_Byte_Shft_Reg[0]);
	      end  // Got_Address_Match
	    else if (VERBOSE) $display("Invalid device address", $time);
...
//************************************************************************
  ...
  always @ (STOP_condition) begin: STOP_Block
    if (S_START ==1) begin S_START = 0; S_STOP = 1; disable STOP_Block; end
    else begin
    ...
    S_START = 0; 
    S_STOP = 1;    Valid_Address_flag = 0;
    Exec_Seq_Read_flag = 0;
    Exec_Random_Read_flag = 0;

    `ifdef AT34C02	// Special tratment for AT34C02
      if ((AT34C02_SWP_enabled == 0) && (AT34C02_SWP_reg == 1)) begin 
        AT34C02_SWP_enabled = 1;
        Exec_Page_Write_flag = 1; 
      end
    `endif

    if (Exec_Page_Write_flag) begin 	// 2-11-2003
      if (VERBOSE) $display ("Time:", $realtime);
      if (VERBOSE) $display ("STOP DETECTED - TIME OUT FOR PAGE WRITE");
      ...
      if (VERBOSE) $display ("END DATA TRANSMISSION");
      disable Data_Transmission;
    end
    end
  end
...
always @ (negedge SCL) begin: Watch_for_Random_START_trigger
  if(S_START && First_START_flag)begin

    if(VERBOSE) $display(">>> Detected random START ? protocol interupt", $time);
...
task Get_a_Byte;	// Reads data on 8 rising edges of SCL
  begin
    ...
    repeat (8) begin
      @ (posedge SCL)
      if (VERBOSE) $display ("Within Get_a_Byte");
      if(VERBOSE) $display("Bit received: %H", S_Byte_Shft_Reg[0]);
    end
  end
endtask
...
if (VERBOSE) $display ("Checking for valid upper bits of device address", $time);
`ifdef AT24C1024
  Device_Address = S_Byte_Shft_Reg[7: 2];
  if (Device_Address == SLAVE_ADDRESS) 
  begin if (VERBOSE) $display ("Valid device address for AT24C1024");  
...
`ifdef AT24C512B
  ...
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C512B");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C512
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C512");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C256
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C256");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C128
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C128");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C64
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C64");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C64A
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C64A");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C64B
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C64B");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C64C
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C64C");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C64D
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C64D");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C32
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C32");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C32E
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C32E");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C32A
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C32A");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C16
  ...
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C16");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C16A
  Device_Address = S_Byte_Shft_Reg[7: 4];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C16A");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C08
  ...
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C08");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C08A
  Device_Address = {2'b0, S_Byte_Shft_Reg[7: 3]};
  if (VERBOSE) $display ("SLAVE_ADDRESS =", SLAVE_ADDRESS);
  if (VERBOSE) $display ("Device_Address =", Device_Address);

  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C08A");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C04
  Device_Address = S_Byte_Shft_Reg[7: 2];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C04");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C04A
  Device_Address = S_Byte_Shft_Reg[7: 2];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C04A");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C02
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C02");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C02A
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C02A");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT24C01A
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C01A", $time);  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
...
`ifdef AT34C02
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT34C02");  
      Valid_Address_flag = 1;
  end 
...
task Write_or_Dummy_Write_with_Random_Read;
  begin: Wrapper 
    if (VERBOSE) $display ("Write or Dummy Write with Random Read", $time);
    if (VERBOSE) $display("******************* Check for Dummy Write Sequence");
 
    Get_Lower_Device_Address_Bits_and_ACK_to_Master; 

    if (VERBOSE) $display("Device address bits are loaded");
    if(VERBOSE) $display("addr_reg holds %H",addr_reg);
    ...
    begin: Byte_or_Page_Write
      if (VERBOSE) $display("Byte_or_Page_Write Sequence initiated", $time);
      if (VERBOSE) $display("Checking for WRITE PROTECTION");
      if (WP == 1) begin: WP_Detection
        if (VERBOSE) $display ("************************");
        if (VERBOSE) $display ("Invalid Attempt to Write");
        if (VERBOSE) $display ("Chip is write-protected"); 
        if (VERBOSE) $display ("************************");
        disable Data_Transmission;		// 7/26/2008
      end // WP_Detection

      else begin: Write_Sequence 
        if (VERBOSE) $display("Beginning a WRITE sequence", $time);
        Get_Remainder_of_Byte;
        if(VERBOSE) $display("S_Byte_Shft_Reg holds %H",S_Byte_Shft_Reg);
        if(VERBOSE) $display ("addr_reg holds %H",addr_reg);
        ACK_and_Write_a_Byte;
        if (VERBOSE) $display("Increment the address register", $time);
        Execute_Page_Write;
        if (VERBOSE) $display ("*********************************************");
        if (VERBOSE) $display ("********************** TIMEOUT FOR PAGE WRITE");
        if (VERBOSE) $display ("*********************************************");
       end // Write_Sequence		
...
`ifdef AT34C02 
    ...
    begin
      if(VERBOSE) $display("ACKing and writing a byte", $time);
      @ (negedge SCL); 
      S_ACK = 1; shift_in = 0; S_send_rcvb = SEND;

      @ (posedge SCL)  
      @ (negedge SCL) begin
      S_ACK = 0; shift_in = 1; S_send_rcvb = RECEIVE;
    end
...
`else 
  begin	// not AT34C02
    @ (negedge SCL) S_ACK = 1; shift_in = 0; S_send_rcvb = SEND;

 Write_flag = 1;
    @ (posedge SCL) memory[addr_reg] <= S_Byte_Shft_Reg;
    @ (negedge SCL) S_ACK = 0; shift_in = 1; S_send_rcvb = RECEIVE;
    Write_flag = 0;
    -> incr_word_addr;
  end
`endif
...
task Get_Remainder_of_Byte;
  begin
    if (VERBOSE) $display("Getting remainder of byte");
    repeat (7) begin
      @ (posedge SCL)
      if(VERBOSE) 
...
task Get_Lower_Device_Address_Bits_and_ACK_to_Master;  
  ...
  if (VERBOSE) $display("Getting lower device address bits and ACK to Master", $time);
  ...
  @(negedge SCL) 	
  S_ACK = 0; shift_in = 1; // Ends ACK on second drop of SCL 
  S_send_rcvb = RECEIVE;
  end
endtask
...
task Get_Word_Address_Bytes;
  begin
  if(VERBOSE) $display("Get_Word_Address_Bytes");
  case (MEM_SIZE)
   MEM_SIZE_1MEG_BIT,	 
   MEM_SIZE_500K_BIT,	 
   MEM_SIZE_256K_BIT,	 
   MEM_SIZE_128K_BIT,	 
   MEM_SIZE_64K_BIT,		 
   MEM_SIZE_32K_BIT: begin 
     Get_MSB_Byte_and_ACK; shift_in = 1; Get_LSB_Byte_and_ACK; end
   default: begin Get_LSB_Byte_and_ACK; end
  endcase
end
endtask 
...
task Begin_a_Read_Sequence;
  begin
	  if (VERBOSE) $display("Beginning a READ sequence", $time); 
        @ (negedge SCL)
        shift_in = 0;
        NO_ACK_flag = 0;
        if (Device_Address_Active) Check_for_Valid_Upper_Bits_of_Device_Address;
	  if (Valid_Address_flag)
          begin:  Address_Match 
            if (VERBOSE) $display ("Upper_Bits_of_Device_Address_Match");

            case (S_Byte_Shft_Reg[0]) 
              0: begin Write_or_Dummy_Write_with_Random_Read;// Should not occur
                   if (VERBOSE) $display ("********* ERROR *****"); 
                 end
              1: Current_Address_and_Sequential_Read; // Read after dummy write
             endcase
          end  // Address_Match
   end
endtask
...
task Execute_Page_Write;   // Added 1-27-2003
  begin
	if (VERBOSE) $display("time:", $realtime);
      if (VERBOSE) $display("*********************************************");
      if (VERBOSE) $display("*********************************************");
      if (VERBOSE) $display("********* Execute_Page_Write is Writing Bytes");
      if (VERBOSE) $display("*********************************************");
      if (VERBOSE) $display("*********************************************");
      if (VERBOSE) $display("************************************", $time);
    `ifdef AT34C02 begin
      if ((AT34C02_SWP_enabled && (addr_reg > 127))||(AT34C02_SWP_reg == 0))
        begin
          shift_in = 1;
          Exec_Page_Write_flag = 1;	
          forever begin: Writing_Bytes
...
begin
  ...
  if (VERBOSE) $display("Beginning Current_Address_and_Sequential_Read", $time);
  Load_Slave_Shift_Register_and_Send_ACK;
  @(posedge SCL)    
  ld_S_Byte_Shft_Reg = 0;
  shift_out = 1; 
  @(negedge SCL) 
  forever begin: Current_or_Sequential_Read
  ...
  ->incr_addr_reg;
  @ (posedge SCL)
    if (SDA_in > 0) begin :Got_NO_ACK_from_Master
    ...
    else begin: Got_ACK_from_Master  //  bit was 0
...
begin
  ...
  @(posedge SCL)
      ...
      @ (negedge SCL)
      if(S_START) begin
        if(VERBOSE) $display("Protocol interupted by START", $time);
       disable Data_Transmission.Outer_Block.Inner_Block;
      end
      S_ACK = 0; 
end
...
begin
  if (VERBOSE) $display ("got ACK from master");
  @ (negedge SCL)
  if(S_START) begin
    if(VERBOSE) $display("Protocol interupted by START", $time);
    disable Data_Transmission.Outer_Block.Inner_Block;
...
task NO_ACK_So_Wait_for_STOP_Condition; // Modified for protocol interupt
  begin
    NO_ACK_flag = 1; 
    if (VERBOSE) $display ("Got_NO_ACK_from_Master ", $time);
    ld_S_Byte_Shft_Reg = 0;
    @ (STOP_condition)
    NO_ACK_flag = 0;
    S_START = 0;	// 04/24/2006
...
task Get_MSB_Byte_and_ACK;  // Modified for protocol interupt
  begin
    if (VERBOSE) $display("Get_MSB_Byte_and_ACK", $time);
    Get_a_Byte;
    @ (negedge SCL) 
    shift_in = 0;
    ...
    S_ACK = 1; 				// Send ACK to master
    @ (negedge SCL) 
    if (VERBOSE) $display ("Loading MSB address byte", $time);
...
task Get_LSB_Byte_and_ACK;		//Modified for protocol interupt
  begin
    if (VERBOSE) $display("Get_LSB_Byte_and_ACK", $time);
    //shift_in = 1;
    Get_a_Byte;
    @ (negedge SCL);
    shift_in = 0;
    ...
    S_ACK = 1;                      // Send ACK to master
    @ (negedge SCL) 
    if (VERBOSE) $display("Loading LSB address byte", $time);
